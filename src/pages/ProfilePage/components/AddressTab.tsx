...
export interface NormalisedAddress {
  docId: string; street: string; city: string; state: string;
  zip: string; country?: string; isDefault: boolean; userId: string; id: string;
}

interface AddressesTabProps { addresses: NormalisedAddress[]; loading: boolean }
  ...
  return (
    <Card className="p-4 sm:p-6 lg:p-8 border-primary/10">
      <div className="flex items-center justify-between mb-5 sm:mb-6">
        <div>
          <h2 className="text-lg sm:text-xl lg:text-2xl font-bold">Delivery Addresses</h2>
          <p className="text-xs text-muted-foreground mt-0.5">Your saved delivery locations</p>
        </div>
        <Button size="sm"
      ...
      {loading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          {[1, 2].map((i) => <div key={i} className="h-28 sm:h-32 rounded-xl bg-muted/30 animate-pulse" />)}
        </div>
      )}
              ...
              <Card className="p-3 sm:p-4 border-primary/10 hover:border-primary/25 transition-all group h-full">
                ...
                <div className="flex items-start gap-2 sm:gap-2.5">
                  ...
                  <div className="text-sm leading-relaxed space-y-0.5 min-w-0">
                    ...
                    {addr.country && <p className="text-muted-foreground text-xs">{addr.country}</p>}
                  </div>
                </div>
              </Card>
            </motion.div>
