...
import { addToCart, clearCart, decreaseQuantity, increaseQuantity, removeFromCart, selectCartItems } from "@/store/FoodCart/actions";
...
export default function CartPage() {
  ...
  const loadAddresses = async () => {
    const res: any = await DomoApi.ListDocuments(CollectionName.FOODAPP_ADDRESSES);
    const mapped = res
  ...
  const handleSubmitOrder = async () => {
    ...
    await Promise.all(cartItems.map((item: any) => CartService.delete(item.docId)));
    dispatch(clearCart());
    setConfirmOpen(false);
  };
 
  const handleIncrease = async (item: any) => {
    await CartService.update(item.docId, { ...item, quantity: item.quantity + 1 });
  ...
  return (
    <div className="min-h-screen" style={{ background: "oklch(0.12 0 0)" }}>
          ...
          <div className="grid lg:grid-cols-[1fr_340px] gap-6">
            ...
            <motion.div layout initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              <div className="rounded-2xl overflow-hidden sticky top-24"
                ...
                {/* Summary Header */}
                <div className="px-6 pt-6 pb-4" style={{ borderBottom: "1px solid oklch(1 0 0 / 8%)" }}>
                  <p className="text-xs font-semibold tracking-widest uppercase mb-0.5"
                    style={{ color: "oklch(0.75 0.18 85)" }}>Billing</p>
                ...
                <div className="p-6 space-y-3">
                  {[
                    ...
                    <div key={row.label} className="flex justify-between text-sm">
                      <span style={{ color: "oklch(0.6 0 0)" }}>{row.label}</span>
                      <span className="text-white font-medium">{row.value}</span>
                    </div>
                  ))}
 
                  <div className="pt-3 mt-1" style={{ borderTop: "1px solid oklch(1 0 0 / 10%)" }}>
                    <div className="flex justify-between items-center">
                      <span className="font-semibold text-white">Total</span>
                      <span className="text-2xl font-bold" style={{ color: "oklch(0.75 0.18 85)" }}>
                        ₹{grandTotal}
                      </span>
                    </div>
                  </div>
                  ...
                  {selectedAddress && (
                    <div className="rounded-xl p-3 mt-2"
                      ...
                      <div className="flex items-start gap-2">
                        <MapPin size={14} className="mt-0.5 shrink-0" style={{ color: "oklch(0.75 0.18 85)" }} />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold mb-0.5" style={{ color: "oklch(0.75 0.18 85)" }}>
                            Delivering to
                          </p>
                          <p className="text-xs text-white truncate">{selectedAddress.street}</p>
      ...
      <AddressSelectSheet
        ...
        onAddNew={() => { setEditingAddress(null); setAddressOpen(true); }}
        onEdit={(addr: any) => { setEditingAddress(addr); setAddressOpen(true); }}
        onContinue={() => { setAddressSelectOpen(false); setConfirmOpen(true); }}
      />
